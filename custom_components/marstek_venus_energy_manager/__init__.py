...
from .pricing import PriceSlot, calculations, notifications
...
class ChargeDischargeController:
    ...
    def _parse_nordpool_prices(self, attrs: dict) -> list:
        """Delegates to pricing.calculations.parse_nordpool_prices."""
        return calculations.parse_nordpool_prices(attrs)

    def _parse_pvpc_prices(self, attrs: dict) -> list:
        """Delegates to pricing.calculations.parse_pvpc_prices."""
        return calculations.parse_pvpc_prices(attrs)

    def _parse_ckw_prices(self, attrs: dict) -> list:
        """Delegates to pricing.calculations.parse_ckw_prices."""
        return calculations.parse_ckw_prices(attrs)

    def _parse_epex_prices(self, attrs: dict) -> list:
        """Delegates to pricing.calculations.parse_epex_prices."""
        return calculations.parse_epex_prices(attrs)

    def _parse_entsoe_prices(self, attrs: dict) -> list:
        """Delegates to pricing.calculations.parse_entsoe_prices."""
        return calculations.parse_entsoe_prices(attrs)
    ...
    def _calculate_charging_hours_needed(self, deficit_kwh: float) -> float:
        """Delegates to pricing.calculations.calculate_charging_hours_needed."""
        return calculations.calculate_charging_hours_needed(
            deficit_kwh, self.max_contracted_power, self.max_charge_capacity
        )

    def _select_cheapest_blocks(self, slots: list, hours_needed: float, slot_duration_h: float) -> list:
        """Delegates to pricing.calculations.select_cheapest_blocks."""
        return calculations.select_cheapest_blocks(slots, hours_needed, slot_duration_h)

    def _select_cheapest_hours(self, slots: list, hours_needed: float) -> list:
        """Delegates to pricing.calculations.select_cheapest_hours."""
        return calculations.select_cheapest_hours(slots, hours_needed, self.max_price_threshold)
    ...
    async def _send_dynamic_pricing_notification(
        ...
        title, message = notifications.format_dynamic_pricing_notification(
            ...
            max_contracted_power=self.max_contracted_power,
            max_charge_capacity=self.max_charge_capacity,
        )
    ...
    async def _send_predictive_charging_notification(
        ...
        title, message = notifications.format_predictive_notification_message(
            ...
            max_contracted_power=self.max_contracted_power,
            max_charge_capacity=self.max_charge_capacity,
            charging_time_slot=self.charging_time_slot,
        )
